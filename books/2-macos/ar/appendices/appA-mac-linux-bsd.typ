...
ماك ولِينُكس وBSD ثلاثةُ أنظمةٍ من عائلة يونِكس، فإن جلستَ أمام أيٍّ منها وجدتَ الصدفة نفسها والأوامر الأساسيّة نفسها: `cd` و`ls` و`grep` و`ssh`. لكنّها تفترق في التفاصيل الدقيقة؛ فأداةُ تثبيت البرامج تختلف، وطريقةُ إدارة الخدمات تختلف، بل إنّ أمرًا مألوفًا كـ `sed` قد يرفض عَلَمًا يعمل هنا ويأباه هناك. سببُ هذا الافتراق نسبٌ مزدوج: ماك وBSD يرثان كثيرًا من أدواتهما من فرع BSD العريق، بينما يستعمل لِينُكس غالبًا أدوات مشروع GNU؛ فأمرٌ واحدٌ بالاسم قد يكون برنامجين مختلفين في السلوك.
...
هنا تفترق الأنظمة أكثر ما تفترق؛ فلكلٍّ منها آليّةُ إقلاعٍ وإدارة خدماتٍ مختلفةٌ تمامًا: ماك يستعمل launchd عبر `launchctl`، ولِينُكس الحديث يستعمل systemd عبر `systemctl`، وBSD يستعمل نظام rc.d عبر `service` وملفّ `/etc/rc.conf`.
...
#note[
  على ماك تُدار خدمات Homebrew بأمرٍ أبسط: `brew services start NAME` و`brew services list`؛ وهو غلافٌ مريحٌ فوق launchd للبرامج التي ثبّتَّها بـ `brew`.
]
