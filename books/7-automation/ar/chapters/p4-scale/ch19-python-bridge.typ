...
#objectives((
  ...
  [تستخدم مكتبة المسارات (`pathlib`) ومحلل الوسائط (`argparse`) ومكتبة جيسون (`json`) بدل معالجة النص يدويًا.],
...
- ملفات جيسون (`JSON`) متداخلة كثيرة.
...
```text
guardian.py --config PATH plan [--json]
guardian.py --config PATH run
guardian.py --config PATH doctor
```
...
الدالة `main` تعيد حالة خروج، ومكتبة المسارات (`pathlib`) تتعامل
مع المسار بوصفه كائنًا، ومحلل الوسائط (`argparse`) يبني واجهة
ومساعدة.
...
لا نكتب محلل جيسون ولا نتولى تهريب محارف العربية يدويًا.
...
4. أبقِ برنامجًا وسيطًا صغيرًا إن كانت الجدولة تعتمد اسمًا قديمًا.
