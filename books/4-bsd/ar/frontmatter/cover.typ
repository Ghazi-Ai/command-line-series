...
#page(width: 148mm, height: 210mm, margin: 0pt, header: none, footer: none, numbering: none, {
  ...
  // الفنّ يملأ الصفحة
  place(top + left, image("/books/4-bsd/ar/assets/cover-front.png", width: 148mm, height: 210mm))

  // العنوان أعلى الصفحة، فوق الهالة
  place(top + center, dy: 15mm, block(width: 100%, {
    set align(center)
    text(font: FONT.displayAr, size: 30pt, weight: 800, fill: rgb("#5E1A13"))[مِن الصِّفر \ إلى العِفريت]
    v(5mm)
    text(font: FONT.bodyAr, size: 12pt, weight: 600, fill: rgb("#7A241B"))[الدليل الشامل إلى FreeBSD وعالم BSD]
  }))
