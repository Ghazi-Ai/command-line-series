...
#goal[أن تعرف سطحَ هجومك: على أيِّ المنافذ يستمع نظامُك.]
#doit[اعرض المنافذَ التي يستمع عليها نظامك الآن.]
#win(`Get-NetTCPConnection -State Listen`)
#linux(`sudo ss -tulpn`)
#mac(`sudo lsof -i -P | grep LISTEN`)
#bsd(`sockstat -l`)
#diff[«سطحُ الهجوم» هو المنافذُ المفتوحةُ للعالم؛ كلُّ منفذٍ بابٌ محتمَل. لِينُكس أداتُه `ss -tulpn` (خَلَفُ `netstat`)، وماك `lsof -i`، وBSD أداتُه الأنيقةُ الأصيلةُ `sockstat -l`، وويندوز `Get-NetTCPConnection`. قاعدةُ التقوية الذهبيّة: أغلقْ كلَّ منفذٍ لا تحتاجه.]
